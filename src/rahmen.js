...
// Kursbuch – Komplett-Skript für Scriptable (alles in einer Datei)
// Die Oberfläche ist direkt eingebettet; es wird KEINE kursbuch.html mehr benötigt.
// Daten liegen lokal: Dateien-App → Auf meinem iPad → Scriptable → KursbuchDaten

const fmLokal = FileManager.local();
const DATEN_ORDNER = fmLokal.joinPath(fmLokal.documentsDirectory(), "KursbuchDaten");
const SICHERUNGS_ORDNER = fmLokal.joinPath(DATEN_ORDNER, "Sicherungen");
const DATEN_DATEI = fmLokal.joinPath(DATEN_ORDNER, "kursbuch-daten.json");
const MAX_SICHERUNGEN = 10;
...
function sicherungAnlegen() {
  if (!fmLokal.fileExists(DATEN_DATEI)) return;
  if (!fmLokal.fileExists(SICHERUNGS_ORDNER)) fmLokal.createDirectory(SICHERUNGS_ORDNER, true);
  const ziel = fmLokal.joinPath(SICHERUNGS_ORDNER, "kursbuch-" + zeitstempel() + ".json");
  fmLokal.copy(DATEN_DATEI, ziel);
  const alle = fmLokal.listContents(SICHERUNGS_ORDNER)
    .filter(n => n.startsWith("kursbuch-") && n.endsWith(".json")).sort();
  while (alle.length > MAX_SICHERUNGEN) {
    const alt = alle.shift();
    fmLokal.remove(fmLokal.joinPath(SICHERUNGS_ORDNER, alt));
  }
}
function datenSchreiben(jsonText) {
  if (!fmLokal.fileExists(DATEN_ORDNER)) fmLokal.createDirectory(DATEN_ORDNER, true);
  const tmp = DATEN_DATEI + ".neu";
  fmLokal.writeString(tmp, jsonText);
  if (fmLokal.fileExists(DATEN_DATEI)) fmLokal.remove(DATEN_DATEI);
  fmLokal.move(tmp, DATEN_DATEI);
}

// ---------- Daten laden und einsetzen ----------
let daten = "null";
if (fmLokal.fileExists(DATEN_DATEI)) {
  daten = fmLokal.readString(DATEN_DATEI) || "null";
...
const STARTSEITE =
  ...
  "<b>Kursbuch wird geladen …</b><br><br>Fassung " + VERSION +
  ...
  while (laeuft) {
    ...
    if (msg.typ === "save") {
      ...
      try {
        ...
        const pfad = fmLokal.joinPath(DATEN_ORDNER, name);
        if (!fmLokal.fileExists(DATEN_ORDNER)) fmLokal.createDirectory(DATEN_ORDNER, true);
        fmLokal.write(pfad, Data.fromBase64String(msg.base64));
      ...
      try {
        ...
        if (pfade && pfade.length) {
          const inhalt = FileManager.local().readString(pfade[0]);
      ...
      try {
        const exportPfad = fmLokal.joinPath(DATEN_ORDNER, "kursbuch-sicherung-" + zeitstempel() + ".json");
        const stand = await wv.evaluateJavaScript("window.__KB_getState()", false);
        fmLokal.writeString(exportPfad, stand);
        await DocumentPicker.export(exportPfad);
        fmLokal.remove(exportPfad);
      } catch (e) { /* abgebrochen */ }
